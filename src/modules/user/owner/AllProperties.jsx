...
const OwnerAllProperties = () => {
  ...
  return (
   <div className="p-6">
  <div className="overflow-x-auto rounded-lg shadow-2xl border border-slate-700 bg-slate-900/70 backdrop-blur-md">
    <table className="w-full text-sm text-left text-slate-200">
      ...
      <tbody>
        {allProperties.map((property) => (
          <tr
            key={property._id}
            className="border-b border-slate-700 hover:bg-slate-800/60 transition duration-200"
          >
            ...
            <td className="px-4 py-3 flex gap-2 justify-center">
              <button
                ...
                className="px-3 py-1 text-sm border border-cyan-500 text-cyan-300 rounded-lg hover:bg-cyan-500/20 transition"
              >
                Edit
              </button>
              <button
                onClick={() => handleDelete(property._id)}
                className="px-3 py-1 text-sm border border-rose-600 text-rose-300 rounded-lg hover:bg-rose-600/20 transition"
              >
                Delete
              </button>
            </td>
  ...
  {show && (
    <div className="fixed inset-0 flex items-center justify-center bg-black/70 backdrop-blur-sm z-50">
      <div className="bg-slate-900/90 border border-slate-700 text-white w-full max-w-xl p-6 rounded-xl shadow-2xl">
        <h3 className="text-2xl font-bold mb-6 text-cyan-300">Edit Property</h3>
        <form
          onSubmit={(e) => {
            e.preventDefault();
        ...
        >
          <input
            ...
            className="border border-slate-700 bg-slate-900/70 text-white px-3 py-2 w-full rounded-lg focus:ring-2 focus:ring-cyan-500 placeholder-slate-400"
          />
          <input
            type="text"
            name="propertyAdType"
          ...
          <input
            ...
            className="border border-slate-700 bg-slate-900/70 text-white px-3 py-2 w-full rounded-lg cursor-pointer file:mr-3 file:px-3 file:py-1 file:rounded-md file:border-0 file:bg-cyan-600 file:text-white hover:file:bg-cyan-500"
          />
          ...
          <div className="flex justify-end gap-3 mt-4">
            <button
              type="button"
              onClick={handleClose}
              className="px-4 py-2 border border-slate-600 rounded-lg hover:bg-slate-700/50 transition"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="px-4 py-2 bg-cyan-600 text-white rounded-lg hover:bg-cyan-500 transition shadow-lg"
            >
              Save Changes
            </button>
          </div>
