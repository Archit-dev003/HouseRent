import axios, { assetUrl } from "../../config/api";
    ...
    <div className="p-6 text-white">
      ...
      <div className="flex gap-4 items-center mb-6">
        <input
          ...
          className="bg-slate-900/60 border border-slate-700 p-2 rounded w-1/3 text-white placeholder-slate-400 focus:ring-2 focus:ring-cyan-500"
        />
        <select
          value={filterPropertyAdType}
          onChange={(e) => setPropertyAdType(e.target.value)}
          className="bg-slate-900/60 border border-slate-700 p-2 rounded text-white focus:ring-2 focus:ring-cyan-500"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredProperties.length > 0 ? (
          filteredProperties.map((property) => (
            <div
              key={property._id}
              className="bg-slate-900/60 border border-slate-700 rounded-lg shadow-lg hover:shadow-cyan-500/30 transition transform hover:-translate-y-1 overflow-hidden"
            >
              <img
                ...
                {property.isAvailable === "Available" ? (
                  loggedIn ? (
                    <button
                      ...
                      className="mt-3 w-full bg-cyan-600 text-white py-2 rounded hover:bg-cyan-500 transition"
                    >
                      Get Info / Book
                    </button>
                  ) : (
      ...
      {showModal && selectedProperty && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/70 z-50 backdrop-blur-sm">
          <div className="bg-slate-900 p-6 rounded-lg w-full max-w-2xl relative border border-slate-700 shadow-xl">
            <button
            ...
            >
              <input
                ...
                required
                className="bg-slate-900 border border-slate-700 p-2 w-full rounded text-white placeholder-slate-400 focus:ring-2 focus:ring-cyan-500"
              ...
              <button
                type="submit"
                className="w-full bg-cyan-600 text-white py-2 rounded hover:bg-cyan-500 transition"
              >
                Book Property
              </button>
            </form>
